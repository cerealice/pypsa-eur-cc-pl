...
def compute_eu_green_share(n):
    ...
    return green_eu / total_eu if total_eu > 0 else 0
...
def get_steel_prod_eu(network):
    ...
    return pd.Series({
        'Scrap-EAF': (steel_eaf * share_scrap).sum() / 1e3,
        'Green El H2-EAF': (steel_eaf * share_dri_h2 * shares["Green El"]).sum() / 1e3,
        'Grey El H2-EAF': (steel_eaf * share_dri_h2 * shares["Grey El"]).sum() / 1e3,
        'Blue H2-EAF': (steel_eaf * share_dri_h2 * shares["Blue"]).sum() / 1e3,
        'Grey H2-EAF': (steel_eaf * share_dri_h2 * shares["Grey"]).sum() / 1e3,
        'CH4-EAF': (steel_eaf * share_dri_ch4).sum() / 1e3,
        'BOF': steel_bof.sum() * share_uncaptured / 1e3,
        'BOF + TGR': steel_bof.sum() * share_captured / 1e3
    })
...
def get_ammonia_prod_eu(n):
    ...
    ammonia_links = n.links[n.links['bus1'].str.contains("NH3", case=False, na=False)]
    if ammonia_links.empty:
        return pd.Series({"Green El H2": 0.0, "Grey El H2": 0.0, "Blue H2": 0.0, "Grey H2": 0.0})

    total_p1 = -n.links_t.p1[ammonia_links.index].sum(axis=0) * timestep
    ...
    return pd.Series({
        "Green El H2": total_mt * shares["Green El"].mean(),
        "Grey El H2": total_mt * shares["Grey El"].mean(),
        "Blue H2": total_mt * shares["Blue"].mean(),
        "Grey H2": total_mt * shares["Grey"].mean()
    })
...
def get_methanol_prod_eu(n):
    ...
    methanol_links = n.links[n.links['bus1'].str.contains("methanol", case=False, na=False)]
    methanol_links = methanol_links.loc[methanol_links.index.str.contains('methanolisation'), :]
    if methanol_links.empty:
        return pd.Series({"Green El H2": 0.0, "Grey El H2": 0.0, "Blue H2": 0.0, "Grey H2": 0.0})

    total_p1 = -n.links_t.p1[methanol_links.index].sum(axis=0) * timestep
    total_mwh = total_p1.sum()
...
def get_hvc_prod_eu(n):
    ...
    if hvc_links.empty:
        return pd.Series({"Fossil naphtha": 0.0, "Bio naphtha": 0.0, "Fischer-Tropsch": 0.0})
...
# =============================================================================
# PLOT SINGLE NETWORK: ONE COLUMN PER TECHNOLOGY
# =============================================================================

fig, axes = plt.subplots(
    1, len(technologies),
    figsize=(3.5 * len(technologies), 4.5),
    sharey=False
)
...
for i, tech in enumerate(technologies):
    ...
    prod_df = get_tech_production_components(n, tech)
    subtypes = get_subtypes_for_tech(tech)

    bottom = 0.0
    for subtype in subtypes:
        value = prod_df.get(subtype, 0.0)
    ...
    ax.set_xticks([0])
    ax.set_xticklabels([str(year)], fontsize=11)
    ...
    if i == 0:
        ax.set_ylabel("Production [Mt/yr]", fontsize=12)

    ax.grid(True, axis='y', linestyle="--", alpha=0.3)

fig.suptitle(f"{scenario} - {year}", fontsize=14, y=1.02)
...
legend_handles = []
for j, (tech_name, subtypes) in enumerate(legend_tech_groups.items()):
    legend_handles.append(mpatches.Patch(color='none', label=tech_name))
    for subtype in subtypes:
        if subtype in feedstock_colors:
            legend_handles.append(mpatches.Patch(color=feedstock_colors[subtype], label=subtype))
    if j < len(legend_tech_groups) - 1:
...
plt.savefig(f"graphs/eu_prod_{scenario}_{year}.png", dpi=300, bbox_inches='tight')
plt.show()